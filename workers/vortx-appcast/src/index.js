...
export class FeedCoordinator {
  ...
  async fetch(request) {
    ...
    return this.state.storage.transaction(async (storage) => {
      ...
      const legacy = !current ? legacySnapshot : null;
      const predecessor = current || (legacy?.manifest?.generation && legacy?.sourceText && legacy?.appcastText && legacy?.receiptSha256 ? legacy : null);
