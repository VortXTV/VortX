...
- **Android: the VortX account with realtime sync.** Sign in on Android and your library, Continue Watching, settings, and profiles sync live with your other devices, the same account and encryption as everywhere else. Android phone and TV.
...
- **Android: Smart Source, ranking, and source pins.** Preference-driven source ranking with auto-pick, mirroring the Apple behavior. Android phone and TV.
- **Android: offline downloads.** The downloads path is reachable, with a real queue, storage policy, and pause and resume semantics. Android phone and TV.
- **Android: IPTV Live.** A Live tab that takes M3U and Xtream playlists with XMLTV guide data, mirroring the Apple design. Android phone and TV.
- **Android: Picture-in-Picture, gesture controls, and system media integration.** PiP, swipe gestures for brightness, volume, and seeking, and lock-screen and notification-shade media controls with audio-focus handling. Android phone.
- **Android: 1080p client-side trailers, add-on management, and quality of life.** Full-HD YouTube trailers, add-on reorder and toggle, automatic subtitle add-on fetching, mark watched from a card, and a player lock. Android phone and TV.
- **Apple: a player lock.** Lock the player controls so a bumped remote or touch cannot pause, seek, or exit. Apple TV, iPhone, iPad, and Mac.
- **Apple: Clear Continue Watching, and account-move groundwork.** A one-tap Continue Watching clear, and the app starts seeding the upcoming move to the VortX bundle identity so the switch carries your data. Apple TV, iPhone, iPad, and Mac.
...
- **Web: a TV surface for LG and Samsung.** The web app gains its first webOS and Tizen surface, arriving at web.vortx.tv with its next deploy. Web.
...
- **Progress saves to the episode that is actually playing.** Account progress used to key on the episode the player was launched with, so a binge session could write a later episode's position onto the first one; it now keys on the current episode, always. Apple TV, iPhone, iPad, and Mac.
