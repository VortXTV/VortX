...
- **Android: Picture-in-Picture, gesture controls, and system media integration.** The player supports PiP, swipe gestures for brightness, volume, and seeking, and appears in the notification shade and on the lock screen with full media controls and audio-focus handling. Android.
- **Android: IPTV Live.** A Live tab that takes M3U and Xtream playlists with XMLTV guide data, mirroring the Apple design. Android.
- **Android: real downloads.** A proper download queue with storage policy and pause and resume semantics. Android.
