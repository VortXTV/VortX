...
A fix beta over Beta 6. Fresh Dolby Vision plays on Apple TV no longer skip roughly fourteen seconds at launch, resumed titles rebuild from the saved position, and display mode no longer flickers through repeated switches at the start of a title. The community (Singularity) source pool is collecting and appearing again, after four separate causes kept it empty. The diagnostic log you send us carries far less about you, and only you can fetch it. And debrid credentials are now scoped to the VortX account that owns them, so signing out and into a different account on the same device can no longer inherit the previous account's debrid keys.

### Fixed

- **Fresh Dolby Vision plays no longer skip the opening fourteen seconds.** The player was never told where to begin, so it applied the rule meant for live streams and skipped ahead by three segment lengths before the first frame you asked for. It now starts at the requested position. Apple TV.
...
- **Debrid credentials no longer cross accounts on a shared device.** Each service's API key was stored in one entry shared by the whole device, and signing out cleared your VortX sign-in without clearing it, so the next account to sign in on that device inherited your debrid credentials and could spend them. Each key is now stored under the VortX account that owns it: signing out releases the key rather than deleting it, so it comes back if you sign back into the same account, and a different account can never read it. An existing key is moved into your account automatically the first time this runs, with no need to paste it again. Apple TV, iPhone, iPad, and Mac.
