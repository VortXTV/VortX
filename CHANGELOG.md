...
- **Dolby Vision now shows as Dolby Vision on Apple TV, not HDR10.** When a Dolby Vision file played through the built-in (libmpv) player, a Dolby Vision torrent, or a Dolby Vision MKV the true-DV remux lane could not take, the app asked the TV for HDR10 and the TV badged HDR even though the picture was Dolby Vision. The built-in player now asks the Apple TV for its Dolby Vision display mode on any Dolby Vision stream (the same thing the well-known players do on their decoded-MKV lane), so the TV lights Dolby Vision. This is the fix for the long-standing "DV plays as HDR10 on Apple TV" report, and it catches every Dolby Vision file that lands on the built-in player (torrents included), not just the ones that reach the remux lane. The display mode also follows a source or episode change now (the flag is read from the currently-playing stream, not the launch stream), so switching to a non-DV source drops back to HDR10 and switching to a DV source re-engages it. Needs Match Content / Match Dynamic Range on in tvOS Settings, Video and Audio. Plain HDR10 content is unchanged (it still requests HDR10). tvOS only; iPhone, iPad, and Mac have no display-mode switch.
