...
**Diagnostic 9 makes hardware-decoder fallback truthful.** Its 276-drop burst was a 4K HEVC session that the runtime classified as SDR and decoded in software, while the diagnostic's true HDR10 interval used VideoToolbox and recorded no sampled drop burst. The player now keeps the requested decoder separate from mpv's negotiated decoder, records requested and active values with fallback, codec, and dimensions once per file, and tells the viewer when VideoToolbox was requested but software is active. It does not force a decoder or change renderer timing without codec-profile rejection evidence.
...
**Jumping to the next episode is closer to instant.** The next-episode source is already picked ahead of time, but at the boundary the player still opened it cold, so a binge advance had a visible gap while the new stream connected and probed. Now the handoff pre-opens the pipeline during the episode you are watching. On the Dolby Vision and debrid path (the Apple player and its remux lane) the next episode's transport is fully pre-started before the cut and adopted at the switch, so its first segments are already flowing when it mounts. On the built-in player, whose demuxer cannot be pre-opened without a second decode pipeline a jetsam-bound Apple TV cannot afford, the source is now warmed across a wider prefix (32 MiB, up from 16) that covers what the player actually reads at a cold open including its stream-inspection probe, so the source edge is hot at the cut. Apple TV.

**Scrubbing far across a title never gets stuck.** A jump that lands outside the buffered window empties the forward cache and refills from a cold mid-file range read, and on some sources that read resyncs slowly or wedges outright, leaving the player stuck in buffering with nothing arriving and the picture frozen, until you backed out and started over. Only the roughly thirty-second mid-play stall reload used to recover it. A bounded, progress-aware refill watchdog now runs alongside the post-scrub cache hold: if the refill shows no buffered-ahead growth across a few-second window while still buffering, it re-issues the same jump to drop the stuck read and reopen it, up to twice, so a wedged refill recovers in a few seconds; the mid-play stall reload stays as the backstop, and a healthy but slowly-filling refill is never interrupted. Apple TV.

**Very large 4K files drop even fewer frames while buffering ahead.** The disk-backed read-ahead ramp already stepped its forward depth up only while the picture kept up, but a real burst of dropped frames merely held the current rung. It now backs off a rung on a genuine drop burst to relieve the fill pressure on the frame-presentation thread and climbs again once the picture is clean, and it caps the ramp's ceiling by the live stream bitrate so an extreme-bitrate remux tops out below the full fifteen minutes rather than over-filling the forward cache. Ordinary 4K stays well under that cap and keeps the full depth, and the deep read-ahead target itself is unchanged. Apple TV, iPhone, iPad, and Mac.

**Seeking on a very high-bitrate 4K HDR title no longer leaves playback stuttering.** The read-ahead ramp above eases its forward depth back when a real burst of dropped frames looks like the fill is straining the frame-presentation thread. A jump with the remote, or a scrub, re-decodes from a keyframe and drops a short burst of frames of its own while the picture rebuilds from the new point, and the ramp read that seek burst as fill strain and shrank the forward buffer exactly when the seek most needed it refilled, so the picture hitched until enough clean windows had passed to climb back (which is why skipping ahead again appeared to clear it). The ramp now tells a user seek's own re-decode burst apart from genuine fill strain and holds its depth across a seek window rather than backing off, while a real starvation burst with no user seek still backs off exactly as before. Apple TV, iPhone, iPad, and Mac.

### Add-ons

**A configuration-page link no longer installs a dead add-on.** Some add-ons, Lumio among them, hand you a personalized link only after you open their configuration page, sign in, and enter your debrid key, and the link they mint ends in /manifest.json. Pasting the bare /configure page link instead used to install an add-on that looked fine but returned no sources on every title: VortX appended /manifest.json to the page link, and the add-on framework answered that with a valid-shaped but empty default manifest, so the install reported success while every stream query came back with nothing. VortX now recognises a /configure page link the moment you add it and tells you to finish setting the add-on up and paste the personalized link it gives you, rather than installing a dead copy. A link that redirects to a configuration page, or returns a web page instead of a manifest, is caught the same way. Apple TV, iPhone, iPad, and Mac.
...
**Scrubbing previews show up far more often.** The scrub thumbnail used to report Previews unavailable whenever the position fell in a gap between captured tiles or past what had been captured, because the only nearest-tile fallback covered a downloaded community sheet and the local capture lookup searched at-or-before within a fixed window. The lookup now also serves the nearest captured frame in either direction from this session's own captures (resident first, then an off-thread disk scan), not just from a community sheet, so a preview shows across the whole timeline including a forward drag into not-yet-watched territory, and it holds a loading state while that read runs rather than flashing unavailable. Apple TV, iPhone, iPad, and Mac.
...
**Source picking prefers the better source again.** After a hand-picked source, the remembered pick could outrank a materially better cached debrid or usenet source on later episodes, because the shared launch and advance resolvers applied the sticky bonus at full strength, sized above the within-tier quality spread. A fresh play, a Continue-Watching launch, and binge auto-advance (with its preload) now all use a soft sticky that yields to any better resolution, cache hit, or source tier and only breaks ties among near-identical releases, so a binge stays on your chosen provider while nothing materially better exists and never hops for a marginal gain. The strong sticky is reserved for the same-episode failover, where a source hand-picked for that episode still holds. Apple TV, iPhone, iPad, and Mac.
...
**A Dolby Vision title playing as HDR10 because of that setting now says so.** Until now this was invisible: the decision was written only to an internal log, so an off setting looked exactly like a title that never had Dolby Vision. The player shows a one-off note naming the setting, on a display that can genuinely show Dolby Vision, so it never blames hardware for something a setting decided. The engine's routing log now also records the value and where it came from, so this can never again be worked out by elimination. Apple TV.

**Choosing the Apple player by hand attempts true Dolby Vision.** Picking Prefer AVPlayer with the Dolby Vision for MKV setting off used to play a Dolby Vision MKV through the plain container path with no Dolby Vision at all. An explicit choice of that player is now taken as an explicit request for what that player is for, so it attempts true Dolby Vision on a display that can show it, and falls back exactly as before if the source cannot carry it. The setting still governs automatic routing. Apple TV, iPhone, iPad, and Mac.

**A Dolby Vision title that takes a moment to open no longer drops to HDR10.** Rebuilding a Dolby Vision MKV to play it opens the source first, and that open can legitimately take several seconds, a ten second network read timeout plus stream inspection, before a single frame has been produced. The player's startup check counted only finished output, muxed bytes and published segments, all of which sit at zero during that open, so it saw nothing move and demoted the whole Dolby Vision attempt to the tone-mapped HDR10 player at fifteen seconds even while the source was still opening. The check now also counts the source being read, and it holds off on a stall demote while an open is genuinely in flight, so a slow-but-alive open keeps its patience and plays as true Dolby Vision, while a genuinely dead source still fails soft to HDR10 within a bounded overall limit. Apple TV, iPhone, iPad, and Mac.

### Playback reliability

**Playback no longer spins forever and never recovers.** A mid-play freeze that still read as buffering could leave the picture on a spinner indefinitely, because the tvOS recovery watchdog exempted the buffering state and never intervened. The watchdog now observes through a buffering freeze (via the shared mid-play stall policy the iPhone/Mac players already use): a genuine wait keeps its full patience, but a stuck stream is recovered on its own instead of stranding you on a spinner. Apple TV.

**Marking something watched shows up right away.** Marking a movie or episode watched from its detail page could leave the checkmark on its old value even though the mark was recorded, because the page only republished when a small set of fields changed and the watched flag was not among them. It now republishes on the watched change itself. Apple TV, iPhone, iPad, and Mac.

**A dead source is dropped in seconds, not half a minute.** A source that opens but delivers no first video packet (empty, truncated, or expired) was handed to the backup player, which also got nothing, wasting about thirty seconds. Such a source is now recognised as terminal from its first-packet failure and the app hops straight to your next source. Apple TV, iPhone, iPad, and Mac.

**Rewinding keeps working on long streams.** The stream can build minutes ahead of the playhead; that lead used to eat the window kept behind you, so a rewind could be refused as out of range. The producer now paces itself (pausing when it runs far ahead) so it never collapses your rewind history. Apple TV, iPhone, iPad, and Mac.

**Dolby Vision fallback no longer risks the wrong colours.** The DV-to-plain-player fallback forced a Dolby Vision colour space purely because the title was labelled Dolby Vision, unsafe for every DV variant. It now applies that only when the format's own signalling proves HDR10 compatibility (Profile 8.1), protects Profile 5, and otherwise waits for real decoded colour parameters. Apple TV, iPhone, iPad, and Mac.

**A wrong or placeholder stream is caught, and resume never jumps past the end.** Switching sources mid-play carried your exact position into the new stream. If that stream was a different, much shorter file, an eight-second clip standing in for a two-hour film, the player seeked to a point past its end and landed on the tail of the wrong thing. Two guards now cover this: a resume position is clamped inside the file that is actually playing, and a stream whose length is a small fraction of what the title should run, whether it is a movie or an episode and at any frame rate, is recognised as the wrong asset so the app moves to a working source instead of playing it. Apple TV, iPhone, iPad, and Mac.

**Watch progress keeps tracking after the app sleeps.** For a movie you were playing from a torrent-based debrid source, leaving the app and coming back could quietly stop recording your progress. The engine's playback record could not be rebound for that movie, because the fallback that binds it skipped torrent-origin sources to avoid picking the wrong episode from a season pack, a risk that exists only for an episodic title. The skip is now limited to episodes, so a movie always keeps its library entry rebuilt and its Continue Watching and resume position track through a suspend and re-resolve exactly like everything else. Apple TV, iPhone, iPad, and Mac.

**A finished season no longer drops the series from Continue Watching.** Watching the last episode of a season used to make the app treat the whole series as done and remove it from your Continue Watching row, so a show with a new season waiting could quietly vanish. Removal is now fail-closed: the app keeps the series in Continue Watching on anything short of proven finality, brings it back when the next season becomes available, and never issues a title-wide rewind off an inferred-complete list. Genuine final removal stays owned by the engine, which already drops a series once there is truly nothing left to watch. Apple TV, iPhone, iPad, and Mac.

**Audio now matches what your speakers actually render.** The app read the audio route's top theoretical capability and mixed for that, so a route that reports more channels than it truly presents, some soundbars over HDMI, could come through thin or too quiet. It now uses the channel count the route realises after the audio session is live: Auto derives its policy from the realized layout, Stereo configures a genuine stereo session, a live route change refreshes it, and any stale forced sample-rate is cleared. No blanket gain or normalization was added. Apple TV, iPhone, iPad, and Mac.

### Apple TV playback

**Seeking far ahead shows you it is working, and gets there sooner.** Jumping past what the player had already prepared rebuilds the stream from the new point, and for the whole rebuild the player published nothing at all: the picture held the old frame while the controls still read as playing, so an ordinary wait looked like a hang. That wait now shows the buffering indicator, and play and pause respond the instant you press them rather than after the rebuild lands. The rebuild is quicker too. The first piece of the new stream was being published with about ten milliseconds of margin over what the Apple player needs before it will start, so it was refused, and the next attempt then sat through a fixed retry delay of more than six seconds out of a thirteen second rebuild. It is now published with real margin, which removes that delay outright.

**The player controls and Settings are lighter.** Deciding whether to show the Sources, Quality, and Chapters buttons walked every loaded source, thousands of them on a busy title, twice per redraw and up to a dozen times a second while the controls were up, and asked the player for its chapter list just as often. Those answers are now recomputed only when they can actually change, including the moment new sources arrive. Settings performed six system lookups for installed external players on every redraw and now performs them once when the screen opens. The engine bridge also stops waking every screen when it reports a change that turned out to contain nothing new.

**The dropped frames line in Playback info is honest.** It showed a running total with no sense of time, so 57 dropped frames across a 51 minute film read as a burst happening right then. It now shows the total alongside the current rate, for example 57 total, 0 per minute, using the measurement the player already takes.
...
**The streaming cache now genuinely buffers far ahead on disk for very large 4K files, with bounded memory.** Turned on in Settings, the streaming cache is meant to let the player read minutes ahead of a big 4K stream and hold that read-ahead on storage instead of memory, so you can seek forward without waiting and a network dip never stalls the picture. It never actually did that: it pointed the player at an option name that no longer exists, so the buffer stayed in memory, and it was sized as if that memory buffer were the disk budget, which is what crashed Apple TVs about twenty seconds into a 4K remux back in 0.2.11 and is why the setting has shipped off ever since. Both are fixed. The player now offloads the forward buffer's data to disk and keeps only a tiny index of it in memory, the read-ahead is bounded to a fixed span of viewing time so it can never run multiple hours ahead, and local torrents keep their small in-memory buffer because the built-in server already buffers those to disk. The setting stays off by default, pending on-device soak testing, and turning it on is now safe. Apple TV, iPhone, iPad, and Mac.

### Trailers and sources

**Re-find sources when a title's links have gone stale.** A source list is cached per title, so an add-on that was offline when you opened a title, or whose configuration has since expired, kept showing nothing (or stale links) until you navigated away and back. A new Re-find sources control on the details page, next to the Sources header and in the no-sources state, re-queries every stream add-on from scratch instead of re-serving the cached list, so expired sources are replaced with fresh ones. It also appears in the player when a source fails to load and every known source has already been tried: it re-queries the add-ons and, once fresh sources arrive, retries playback on the best one. It only ever runs on the details page or after playback has stopped, never mid-play, so a binge and its warmed next-episode source are never disturbed, and your remembered source pick still only breaks ties between near-identical releases. The whole control can be switched off from the backend with no app update. Apple TV, iPhone, iPad, and Mac.

**Home-screen preview trailers play again after the app has been in the background.** The trailer that previews under the featured title on the Home screen plays through a small local helper on the device, and that helper claimed a network port once and never released it. After the app slept and woke, the system had reclaimed that port while the helper still believed it held it, so every preview trailer failed silently until the app was fully closed and reopened. The helper now releases its port whenever the app goes to the background and takes a fresh one on the next preview, and it also heals itself if the port is lost for any other reason without waiting for a background and foreground cycle. Apple TV, iPhone, iPad, and Mac.
...
- **Dolby Vision playlists stay inside the video the player can still serve.** The local player advertises only complete segments whose bytes are still resident, uses their real sequence numbers, and keeps the initial bytes until AVPlayer is ready, so it never points at video already discarded. A failed lane falls back immediately when it reports an error, after about fifteen seconds without remux progress, or at a two-minute hard limit if it keeps trickling without a frame. Apple TV.
- **Dolby Vision keeps the frame rate found during stream classification.** If AVPlayer cannot report a rate for the local stream, it no longer substitutes 60 and asks the TV to switch away from a known 23.976 rate. Display requests are remembered per display manager and only after the manager accepts them. Apple TV.
...
- **Watch Stats fills in from your real history.** The Watch Stats screen (Settings, Watch Stats) could sit empty even with a full watch history behind it, because it drew its numbers from a single persisted library file and dropped that file whenever its stored account id did not match the one signed in right now, which happens when you are signed out, have just switched accounts, or are on a device that has not written it yet. It now reads from your account's own live watch history as well, the same history that already powers Continue Watching and the watched checks on your covers, so your hours, titles, movies, series, episodes, top genres, and longest binge appear whenever that history is present. Time for a title only the live history knows about is estimated from its length and play count; the exact per-title time still comes from the engine's own record when it is available. iPhone, iPad, and Mac.
...
- **Apple: Clear Continue Watching, and account-move groundwork.** A confirmed Continue Watching clear, and a dismissible reminder ahead of the planned move to the VortX app identity asking you to sync or export a backup first; preparation, not a carryover guarantee. Apple TV, iPhone, iPad, and Mac.
- **Glass everywhere on Apple.** Add-on pairing, catalog preferences, integrations, import, the update prompt, sign-in, the debrid library, the download quality picker, and watch stats move onto the same glass system as the rest of the app. Apple TV, iPhone, iPad, and Mac.
- **Web: a TV surface for LG and Samsung, in preview.** A first webOS and Tizen surface lands in source, pending packaging and device validation before it reaches web.vortx.tv. Web.

### Fixed

- **The episode you pick is the episode you get.** A series episode could play the wrong video, show a different episode's details, and save progress to a third, most often with season packs, cached debrid sources, or fast episode switching. Episode identity is now proven end to end: a season pack is matched by its exact torrent file identity, a debrid source is bound to the exact resolved file for the exact episode you chose, and when the right file cannot be proven the app refuses and reranks instead of guessing. Fast episode switching can no longer mix the outgoing episode's finish with the incoming episode's start, all the way down to the video engine's own accounting, and finishing an episode while the next one loads marks the finished one watched without hijacking your selection. Apple TV, iPhone, iPad, and Mac.
- **Binge labels can no longer drift.** Switching episodes used to update the title, the episode selector, and the watched marker the moment you picked, while the previous episode was still rendering; everything now publishes atomically at the incoming episode's first frame, so what you see is always what is playing. Apple TV, iPhone, iPad, and Mac.
- **Progress saves to the episode that is actually playing.** Account progress used to key on the episode the player was launched with, so a binge session could write a later episode's position onto the first one; it now keys on the current episode, always. iPhone, iPad, and Mac.
- **A season finale no longer ends the whole show.** Resuming a title directly from Continue Watching used to see only that season, so finishing a season finale could clear the show's Continue Watching entry even with later seasons ahead; resume now sees the full series. iPhone and iPad.
- **Decade shelves have names and curated rows feel fresh.** Decade catalog tiles show their decade instead of a blank, and curated catalogs rotate instead of pinning the same titles. Apple TV, iPhone, iPad, and Mac.
- **Android player, a long list.** Correct end-of-file versus error handling with a stall watchdog and a real loading spinner; landscape lock, correct mpv aspect ratio, and 10-second seeks; the Home hero binds real backdrop artwork; back handling, controls auto-hide, and next-episode; and a bad source never marks an episode watched, auto-retries, then offers a manual fallback. Android phone and TV.
...
- **A picked subtitle can no longer sit on Loading forever.** Choosing an add-on or community subtitle could leave its row saying Loading with nothing applied, and after that every other subtitle choice was silently ignored for the rest of the session. Two causes, both fixed: if the player engine was being swapped at that exact moment (for example the brief Dolby Vision fallback), the download's finish signal was lost and the in-flight marker never cleared, blocking all later picks until you closed the player; and on Apple TV the subtitle panel never refreshed when a download finished, so the row said Loading forever even when the subtitle was already on screen, and a failed download showed nothing at all. The marker now clears itself on any engine change, source switch, or episode change, a subtitle that finishes loading updates the panel immediately, a failure now says so on Apple TV the way it already did on iPhone, and a subtitle that loaded into a torn-down engine is offered again instead of vanishing from the list. Apple TV, iPhone, iPad, and Mac.
...
- **Nuvio import.** A paste-your-add-on flow in Integrations brings a Nuvio user's Stremio-compatible add-ons into VortX through the same SSRF-hardened install path as every other add-on; no curated third-party list and no account (Nuvio has none). Apple TV, iPhone, iPad, and Mac.
- **Cast and crew pages.** Cast members on a title page are tappable and open a person page with bio details and a filmography grid of their other movies and shows, tapping through to each, powered by TMDB person data over the existing signed edge. Apple TV, iPhone, iPad, and Mac.
- **QR backup on Apple TV.** Back up and Restore in Settings carry your account and data to a fresh install via the same short-lived pairing used for add-ons; the code carries only the pairing handle, never your data. Apple TV.
- **Sync restores on one sign-in.** A fresh install now hydrates your add-ons and owner library the moment you sign into your VortX account, from every sign-in path, instead of only after a background cycle, with the never-zero and tri-state-fetch guards intact so a network blip can never wipe a populated library. Apple TV, iPhone, iPad, and Mac.
- **The liquid-glass redesign.** A shared VortX glass material (warm translucent fill, lit top edge, soft shadow, ember active state) now floats the navigation bar, the title hero action buttons, and the in-player controls, on every OS, upgrading to Apple's real Liquid Glass on the newest systems and standing down to an opaque legible surface under Reduce Transparency. Appearance only: every control, its wiring, and the tvOS focus engine are unchanged. Apple TV, iPhone, iPad, and Mac.
- **A corrective visual pass on iPhone and Mac.** A scaling quirk had been shrinking the iOS and Mac type ramp uniformly, to the point where card titles rendered smaller than body text and the whole hierarchy flattened into one size; the ramp is rebuilt so headings, titles, captions, and labels have real contrast again. Browse posters gained the resting shadow and the hover, focus, and press treatment they were meant to have (the code existed but was never applied), so grids and rails have depth instead of reading flat, and spacing rhythm, section headers, the accent color, and the profile picker centering are made consistent across Home, Discover, title pages, and Settings. This is a correction and consistency pass; larger visual direction changes are proposed separately. iPhone, iPad, and Mac.
- **Dolby Vision player parity, plus a mid-play engine switch.** Titles that route to the dedicated Dolby Vision player (the AVPlayer path) now reach feature parity with the built-in player: real scrubbing, periodic progress saves, resume from your last position, watched-at-the-end marking, skip intro or outro, and audio and subtitle track selection. The root cause of the missing behaviors was that a Dolby Vision remux serves an open-ended stream whose length the player could not read, so it treated the whole session as live and disabled everything length-dependent; the player now synthesizes the real duration from the remuxer. A new Player engine control in the in-player settings switches between the built-in player and the Dolby Vision player mid-title, carrying over position, track choices, and speed. Reviewed across six rounds. Apple TV, iPhone, iPad, and Mac.
- **Media servers: Plex, Jellyfin, and Emby.** Connect your servers (Plex PIN link, Jellyfin Quick Connect or password, Emby password) and their movie and show libraries join Home and Discover as catalog rows, while matched titles list your server's own copies as direct-play sources, ranked alongside everything else and badged per server. App-native by design: playback streams directly from your server, credentials stay in the Keychain and the encrypted sync channel, and with no server connected the feature is invisible and makes zero network calls. Matching is deliberately conservative (id-based with a guarded title-and-year fallback) so a wrong file can never impersonate a match. Direct play only for now; transcode negotiation is a later phase. Apple TV, iPhone, iPad, and Mac.
- **A real macOS surface pass.** The profile picker fits and centers all profiles, the Home hero scales to the window and the title-page hero reads near-fullscreen with the artwork pinned while episodes and sources scroll independently, the hero trailer autoplays again, and the player gained an in-app fullscreen toggle (Ctrl-Cmd-F) with a truly edge-to-edge picture. Mac.
- **Play from start on Apple TV.** Replays the exact same source from 0:00, never re-running source selection. Apple TV.
...
- **Batch downloads retry themselves.** A failed episode swaps in the next-best source automatically, once per episode, with honest status. iPhone, iPad, and Mac.
- **Decade catalog cover art**, picked daily from inside each list, served by the catalogs service. Apple TV, iPhone, iPad, and Mac.
- **Android: Phase A foundation, contributed by jbecker-it (#128), his fourth contribution.** The Android app went from a branded skeleton to a real engine-driven client: account sign-in, real catalogs and Continue Watching, add-on management, Discover, Library, Search, and full movie and series detail pages, all over the shared engine bridged through JNI, with a two-flavor build that keeps the Play-Store flavor GPL-clean (verified by APK inspection). Landed adapted onto the com.vortx.android identity with main-thread and credential-logging hardening. Android.
- **Smart Source Selection.** One panel in Settings > Streams gathers source filtering as chips: Prefer words (float matches up), Only words (keep nothing else), and Avoid words with a per-user choice of hiding matches or ranking them down, plus an opt-in auto-pick that starts the best match and a live preview that reorders a real source list as you type. Filters are per-profile and sync with the roster; Kids profiles always hard-hide avoided words regardless of the avoid behavior; the previously scattered filter rows folded into the panel. Apple TV, iPhone, iPad, and Mac.
- **Live TV playlists: M3U and Xtream.** Add an M3U playlist URL or an Xtream portal login and its channels appear in the Live tab with their XMLTV program guide, served through a hosted converter so the app treats them like any other live source. Registration validates the playlist up front, and removing a playlist revokes it everywhere. Apple TV, iPhone, iPad, and Mac.
- **External watch-state sync groundwork (dormant).** The plumbing for syncing watched history and progress with external tracking services ships in this build but stays entirely invisible and makes zero network calls until service credentials are configured in a later build.

### Fixed (pending next build)

- **The Apple TV build installs again.** A missing tvOS deployment target let the build float up to the newest tvOS SDK, so an Apple TV on any older tvOS refused it (a white placeholder icon, then "unable to install"); the tvOS target is now pinned low, so the build installs on any current Apple TV. Apple TV.
...
- **A round of Mac refinements.** Colored action buttons now take the warm glass treatment while their labels stay readable, the round player buttons lost a soft dark halo and the top-row controls read crisply over bright video, a Settings description that was cut off now wraps in full, decade shelves load their cover art, the profile picker fills the window with nothing clipped off the edge, the Home and title heroes reach nearly the whole window, and the leftover grey bar with its dead back control at the top of fullscreen playback is gone. Mac.
- **More Dolby Vision titles keep true Dolby Vision and their Atmos (two more failure modes closed).** A Dolby Vision title whose file name carried release-group text (an unrecognized pseudo file extension) was disqualified from the Dolby Vision lane before it was ever tried; the gate now probes the real bytes instead. And the fixed twenty-second no-first-frame timeout that demoted a heavy, still-downloading 4K Dolby Vision source to the tone-mapped built-in player, losing both Dolby Vision and its Atmos in one step, is now progress-aware: it holds while the remux is genuinely producing frames, up to a generous ceiling, and only gives up on a true stall, so a slow 4K title keeps its Dolby Vision and Atmos. Titles that already worked are byte-for-byte unchanged, verified by adversarial review. Root-caused from a viewer's exported diagnostic log. Apple TV, iPhone, iPad, and Mac.
- **The Dolby Vision player now matches the built-in player everywhere but Dolby Vision itself.** An exhaustive audit closed the remaining gaps, so the AVPlayer path matches the built-in player on playback, seeking, every audio and subtitle control, skip segments, trickplay, scrobbling, binge auto-advance, chapters, and speed; the differences that remain (manual Atmos or passthrough, audio delay, and a hardware-decode toggle) are engine concepts AVFoundation negotiates on its own, so their controls are correctly hidden rather than broken. Apple TV, iPhone, iPad, and Mac.
- **Trailers play in the right language and to the end.** A trailer carrying multiple audio languages now selects your preferred language instead of whichever track was listed first, and the local trailer proxy now advances by the bytes actually delivered (a bounded range from googlevideo can return fewer bytes than asked), so a trailer no longer stops partway through. Apple TV, iPhone, iPad, and Mac.
- **Apple TV binge tracking stays on the right episode.** After auto-playing through several episodes, backing out to the title page could leave it stuck on the episode you started from, and pressing Watch Now on that stale page could play the binge-advanced episode's video paired with the earlier episode's subtitle, while some finished episodes were not marked watched. Root cause: three separate places tracked the current episode (the player, the engine, and the detail page) and never reconciled after a binge or on returning to the page. The title page now refreshes to the episode you actually reached when the player closes, the tvOS source list is scoped to its own episode so it can only ever resolve that episode's stream, the engine player is re-pointed to each new episode as the binge advances (with a guard that a missed re-point records nothing rather than attributing progress to the wrong episode), and Watch Now and subtitles key off the same episode. Apple TV.
- **The streaming server survives a background suspension (#130).** Consistently reported on Apple TV: leaving the app in the background (Home screen, app switch, screensaver) eventually left the embedded server offline with no automatic reconnect, requiring a restart in Settings. Root cause: the system tears down the server's bound local socket during suspension while the node process keeps running, so it returned alive-by-heartbeat but deaf to connections, and a June watchdog that used to re-establish it had been removed for force-closing the port mid-torrent. The re-listen is restored, but now triggered only by a real suspension and gated so it fires only when the port is genuinely refused, never while a stream is being served, and the server status stops falsely reading "running" while it is deaf. A background-task assertion during active loopback playback also reduces how aggressively the app is suspended mid-stream. Apple TV, iPhone, and iPad. Thanks to konrepo for the diagnostic log.
- **A malformed source can no longer crash the app during ranking.** An add-on stream whose name advertised an absurd size or seeder count (far beyond any real release) could overflow an internal calculation and take the app down while a source list was being sorted, which happens on every source screen. Those figures are now clamped at the point they are read, so a single garbage or hostile entry ranks harmlessly instead of crashing. Scoring is unchanged for every realistic value. Apple TV, iPhone, iPad, and Mac.
- **Huge source lists stopped nudging the interface.** The debrid cache-check merge (which grows with services times sources) ran on the interface thread; it now runs in the background with the same ordering, so titles with thousands of sources scroll smoothly while their cached badges resolve. Artwork requests to VortX services also carry signatures now, groundwork for tightening the edge without breaking a single image. Apple TV, iPhone, iPad, and Mac.
- **The embedded streaming server survives Dolby Vision sessions.** Dolby Vision playback stacks a second full media pipeline into the same memory budget the in-app streaming server lives in, and 0.3.13's DV reliability work made that stack real for the first time: a demoted DV attempt could keep holding up to 128MB for another ten to twenty seconds while the built-in player re-opened the same 4K stream on top of it, and the process footprint ratcheted upward after every DV era, so the system could quietly kill the server (or the whole app) on a later torrent play. Cancelling a DV attempt now aborts its network reads within milliseconds and frees its buffer immediately, an attempt still being judged holds far less memory up front, the server re-trims its caches after every DV session, and a server death is now recorded into the exportable diagnostics log (URLs scrubbed) so a field report can finally show one. From a fleet report against 0.3.13. Apple TV, iPhone, iPad, and Mac.
- **The percent-sign crash class is closed everywhere, not just the Home hero.** The launch crash from a title containing a percent sign (fixed for the Home hero in #127) came from add-on and network text being logged unsafely; a full sweep found and hardened every remaining place in the app that did the same, so no title, source name, filename, or error message can crash a log call regardless of what characters it carries. Apple TV, iPhone, iPad, and Mac.
- **The app no longer force-closes at launch when a catalog title contains a percent sign.** The Home hero's diagnostic logging passed addon-supplied titles straight into the log's format position, so a title carrying a % was read as format specifiers and crashed the app before anything drew (EXC_BAD_ACCESS in the hero's enrichment, the exact stack a community crash report captured). All three hero log sites now pass content as values, never as format, and a sweep of the same pattern across the whole app is underway. Fixes #127; thanks to konrepo for converting and attaching the crash log that pinned it. iPhone, iPad, and Mac.

### Fixed (build 176)

- **Scrub-preview uploads stop spamming doomed attempts.** A field log showed 15+ upload failures in one session; the server was healthy and the real watches stored fine, but the client fired an upload every minute from the second frame onward, each predictably declined below the sharing threshold and logged as a failure. The client now predicts the threshold with the server's exact formula and skips only what would certainly be declined, failing open whenever the duration is unknown; the log distinguishes a skip from a real failure. From the same field log as the Dolby Vision series. Apple TV, iPhone, iPad, and Mac.

- **Live channels and sport events can always navigate back on iPhone, iPad, and Mac.** Live-type detail pages had no back affordance (the hero chrome was only overlaid on movie and series pages), edge swipe-back was dead on every hidden-bar detail page, and the Live tab ignored the re-tap that pops other tabs to their root; the only escape was force-quitting. Live pages now wear the standard back chrome, a guarded swipe-back gate restores the edge gesture on all detail pages (refusing during running transitions so it can never corrupt the stack), and the Live tab pops to root on re-tap. Fixes #125. iPhone, iPad, and Mac.
- **The Dolby Vision reliability series from a field diagnostic log (#76).** Five fixes, each pinned by a viewer's exported log: a stalled packaging step (an audio track the muxer cannot parse starving the stream header, previously a silently discarded error) now fails over in about a second instead of twenty; a failed source no longer locks the remaining sources of the title out of Dolby Vision when the viewer picks another (automatic hops stay conservative); an instantly rejected healthy stream gets one automatic retry, with the startup playlists now answered atomically and every serve logged; MP4 files with hev1 or dvhe sample entries are repaired through the remux lane (rewritten to the form Apple's player renders) instead of playing audio over a black screen; and the TV display-mode switch fires only after a stream is confirmed Dolby Vision, with a teardown guard so an abandoned load can never switch the panel after exit. Apple TV, iPhone, and iPad.
- **Raw subtitle metadata no longer appears at the start of subtitle lines.** Builds 0.3.9 and 0.3.10 uploaded community-pool subtitles where comma-less cues fell back to the raw event row (the "0,0,Default" prefixes a viewer reported), and nonstandard subtitle formats could leak fresh; the extractor now splits by each file's own declared format, the display pass strips all seven known leak shapes behind a conservative per-file vote (verified with zero residual against the actual affected pool files), and the verification harness ships in the repo. A server-side cleanup of the old pool entries is prepared separately. Part of #76. Apple TV, iPhone, iPad, and Mac.
- **The Apple TV Home screen loads the whole catalog board.** Home never requested more of the engine's board beyond the first window, capping users with many add-on catalogs at about thirty rows; scrolling near the last section now pages the rest in, in both rails and the poster wall. Apple TV.
- **Source filters follow their profile faithfully.** Filter edits are captured to the active profile as they happen (previously only order changes were), live settings are captured before any profile switch can touch them, and switching to a profile that never recorded a filter applies the documented default instead of silently inheriting the previous profile's values. Apple TV, iPhone, iPad, and Mac.
- **The Android debug build compiles and ships artifacts again.** A dependency's newer Kotlin metadata crashed the compiler, which had masked a real error in the ExoPlayer fallback engine, and the CI artifact glob silently matched nothing after the flavor split; all three fixed. Contributed by jbecker-it in #124, his third contribution. Android.

### Added (build 175)

- **Watched badges on Home and catalog covers (build 175).** The Library's check badge and dimmed cover now appear on every Home rail and browse grid on Apple TV, per profile: the owner reads the engine's own watched bookkeeping (including titles marked watched straight from a catalog), overlay profiles read only their private history, and the lookup is a read-only index that never writes anywhere. Requested in #111's follow-up. Apple TV.
- **Batch season and episode-range downloads (build 175).** Download Season, multi-select, and per-episode badges on the episode list; a sequential resolver picks each episode's best source with the exact ranking a manual download uses, including the debrid search and community pool through their existing rate-limit-safe paths, one resolution in flight at a time; skips are honest and listed, an interrupted batch reports what was left, and Stop works mid-resolve. Fixes #119. iPhone, iPad, and Mac.
- **Offline mode (build 175).** A shared connectivity monitor drives a quiet offline banner on every platform, and an offline launch lands on Downloads (iPhone, iPad, Mac, with Settings as the no-downloads fallback) or the Library (Apple TV). The launch verdict is confirmed before it can route (a Wi-Fi still reassociating at cold launch can never misroute a healthy session), the routing never fires once you have navigated, and mid-session changes only show the banner. Fixes #120. Apple TV, iPhone, iPad, and Mac.
- **Source filters: minimum quality floor, hide unknown quality, preferred audio languages only (build 175).** The floor drops only sources whose parsed quality is known and below it; unknown-quality sources are kept unless the new hide-unknown toggle is on; the audio filter drops only releases that positively advertise a different audio language (subtitle-only labels like korsub or vostfr never trigger it), and the footnote says exactly that. All three sync per profile and to the dashboard like the existing filters. Part of #117. Apple TV, iPhone, iPad, and Mac.
- **Compact source rows (build 175).** A display toggle shows the parsed quality line instead of raw release names, on both the phone and TV source lists. Part of #117. Apple TV, iPhone, iPad, and Mac.
- **Tab bar customization (build 175).** Live, Discover, Library, and Search can each be hidden; Home, Add-ons, and Settings are always present. Hidden tabs unmount (their background work stops), selection heals to Home if the active tab is hidden, the Mac search shortcuts and the offline landing never route to a hidden tab, and the old Show Live TV toggle migrates in. Part of #117. Apple TV, iPhone, iPad, and Mac.
- **A poster-wall layout option for Home on Apple TV (build 175).** Each catalog renders as a vertical poster grid with its header kept, paging in more titles as focus nears the end; Continue Watching stays a rail in both layouts, and each wall is its own focus section so the remote steps predictably between sections. The rails stay the default. Fixes #105. Apple TV.
- **A trailer that cannot play in-app offers the YouTube app on Apple TV (build 175).** The failure path hands the same trailer id to the YouTube app (long-press the trailer button to go straight there), the language-preferred id carries over, and the unavailable note only shows when both paths fail. Part of #95. Apple TV.

### Fixed (build 175)

- **Streaming services no longer vanish when a region lists them under an ad-supported or free tier (build 175).** The catalog queries now cover subscription, ads, and free monetization together (rent and buy stay excluded), verified live against the catalog source: the query can only ever add titles, and free services like Tubi and Pluto TV stop showing near-empty catalogs. Hardens the #114 case beyond the 0.3.13 id-family fix. Apple TV, iPhone, iPad, and Mac.
