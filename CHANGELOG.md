...
- **App downloads no longer carry a build-machine path, and install cleanly.** A leftover absolute link on the build machine had been copied into every release bundle, which embedded a path from that machine inside every sideload download and kept the app from installing on the simulator. Every built bundle is now checked before packaging and the build fails closed if an unsafe link is found, so downloads no longer carry that path. Apple TV, iPhone, iPad, and Mac.
