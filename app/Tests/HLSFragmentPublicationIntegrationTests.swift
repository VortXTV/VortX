...
private struct RealOpenStageResult {
    ...
    let finalLength: Int?
    let finalPayloadMatched: Bool
    let successorBytes: Int
    let successorRemainedOpen: Bool
    let successorPayloadMatched: Bool
...
private func dataMatches(_ lhs: Data, _ rhs: Data) -> Bool {
    ...
    return lhs.withUnsafeBytes { lhsRaw in
        rhs.withUnsafeBytes { rhsRaw in
            guard lhsRaw.count == 0 || (lhsRaw.baseAddress != nil && rhsRaw.baseAddress != nil) else {
                return false
            }
            return lhsRaw.count == 0
                || memcmp(lhsRaw.baseAddress!, rhsRaw.baseAddress!, lhsRaw.count) == 0
        }
    }
}

private func resourceMatches(_ lease: VortXHLSSessionSpool.ResourceLease?,
                             expected: Data,
                             chunkSize: Int = 512 * 1024) -> Bool {
    guard let lease else { return false }
    var offset = 0
    do {
        while offset < expected.count {
            let bytes = try lease.read(maxLength: min(chunkSize, expected.count - offset))
            guard !bytes.isEmpty else { return false }
            let expectedChunk = expected.subdata(in: offset..<(offset + bytes.count))
            guard dataMatches(bytes, expectedChunk) else { return false }
            offset += bytes.count
        }
        return try lease.read(maxLength: 1).isEmpty
    } catch {
        return false
    }
}
...
private func runRealOpenStageScenario(_ result: ScenarioResult) throws -> RealOpenStageResult {
    ...
    let finalPayloadMatched = resourceMatches(lease, expected: result.mediaData)
    lease?.close(now: 0)
    let afterClose = stage.snapshot
    var successorPayloadMatched = false
    if let successorClaim = stage.claim() {
        _ = successorClaim.withBytes { bytes in
            successorPayloadMatched = dataMatches(bytes, successor)
        }
    ...
    return RealOpenStageResult(
        ...
        finalPayloadMatched: finalPayloadMatched,
        ...
        successorPayloadMatched: successorPayloadMatched,
            ...
            if let openGOP = results.first(where: { $0.name == "real >80MiB open GOP" }) {
                ...
                checks.check("real open stage: P commits to the private registry and retains S as the next open stage",
                             staged.closeCommitted
                                 ...
                                 && staged.finalPayloadMatched
                                 && staged.successorBytes > 0
                                 && staged.successorRemainedOpen
                                 && staged.successorPayloadMatched)
