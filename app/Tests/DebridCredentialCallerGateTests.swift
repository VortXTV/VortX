...
private let credentialScopePath = "app/SourcesShared/CredentialScope.swift"
...
private let traktAuthPath = "app/SourcesShared/TraktAuth.swift"
private let simklAuthPath = "app/SourcesShared/SIMKLAuth.swift"
...
private let authCommitWiringRule =
    "auth adoption defaults to Keychain and revalidates after the injected suspension"
...
private let rules: [Rule] = [
    ...
    Rule(name: ownerRule, files: [credentialScopePath, statePath], violations: { files in
        guard let scope = files[credentialScopePath], let state = files[statePath] else {
            return ["missing canonical credential scope or debrid credential state"]
        }
        return require(scope, "enum CredentialScope: Equatable, Hashable, Sendable",
                       "one typed owner identity")
            + requireOrdered(scope, [
            ...
            ], "disjoint device and account namespaces")
            + require(state, "typealias DebridOwnerScope = CredentialScope",
                      "debrid shares the canonical owner grammar")
            + require(state, "let base = \"vortx.debrid.v3.envelope.\" + owner.storageNamespace",
    ...
    Rule(name: coordinatorRule, files: [statePath, resolverPath], violations: { files in
        guard let state = files[statePath], let file = files[resolverPath],
              let install = section(file, start: "private func installIfCurrent(",
                                    end: "private func ensureCurrentSnapshot()") else {
            return ["missing credential store, resolver, or coordinator install section"]
        }
        return require(file, "func reload(snapshot: DebridCredentialSnapshot)", "typed reload API")
            + require(file, "if let onBeforeReloadCommit { await onBeforeReloadCommit(snapshot) }",
                      "production-linked pending-window test seam")
            + require(state, "func compareAndInstall(revision: UInt64, mutation: () -> Void) -> Bool",
                      "credential-publication linearization seam")
            + requireOrdered(install, [
                "credentialStore.compareAndInstall(revision: snapshot.revision)",
                "guard revisionFence.accept(snapshot) else { return }",
                "resolvers = nextResolvers",
                "torboxUsenet = nextTorBoxUsenet",
                "appliedSnapshot = snapshot",
            ], "store-linearized complete resolver generation swap")
            + require(file, "guard revisionFence.accept(snapshot) else { return }",
    ...
    Rule(name: authCommitWiringRule,
         files: [credentialScopePath, traktAuthPath, simklAuthPath], violations: { files in
        guard let scope = files[credentialScopePath],
              let trakt = files[traktAuthPath],
              let simkl = files[simklAuthPath],
              let traktAdopt = section(trakt, start: "func adoptTokens(",
                                       end: "func syncableTokens()"),
              let simklAdopt = section(simkl, start: "func adoptTokens(",
                                       end: "func syncableTokens()"),
              let traktCommit = section(trakt, start: "private func commitIfCurrent(",
                                        end: "private func terminalRefreshFailure("),
              let simklCommit = section(simkl, start: "private func commitIfCurrent(",
                                        end: "private func ensureConfigured()"),
              let traktWrite = section(trakt, start: "nonisolated private func writeTokens(",
                                       end: "nonisolated private func clearTokens("),
              let simklWrite = section(simkl, start: "nonisolated private func writeTokens(",
                                       end: "nonisolated private func clearTokens(") else {
            return ["missing auth adoption, commit, or storage section"]
        }
        var findings = require(scope, "static let keychain = CredentialTokenStorage(",
                               "production Keychain token backend")
            + require(scope, "CredentialScopedKeychain.set(value, for: account, scope: scope)",
                      "Keychain backend owner-scoped write")
        for (file, adopt, commit, write, label) in [
            (trakt, traktAdopt, traktCommit, traktWrite, "Trakt"),
            (simkl, simklAdopt, simklCommit, simklWrite, "SIMKL"),
        ] {
            findings += require(file, "tokenStorage: CredentialTokenStorage = .keychain",
                                "\(label) production initializer defaults to Keychain")
                + requireOrdered(adopt, [
                    "credentialStamp suppliedStamp: CredentialCommitStamp? = nil",
                    "await commitIfCurrent(stamp)",
                    "self.writeTokens(",
                ], "\(label) adoptTokens reaches the common guarded storage path")
                + requireOrdered(commit, [
                ...
                ], "\(label) injected suspension precedes same-turn revalidation and mutation")
                + forbid(write, "await ", "\(label) storage writes suspend after revalidation")
                + require(write, "tokenStorage.write", "\(label) writes through injected production backend")
        }
        return findings
    }),
]
...
private let mutations: [Mutation] = [
    ...
    Mutation(name: "M03 map account storage onto the device namespace", rule: ownerRule,
             path: credentialScopePath,
             find: "return \"account.\\(id.uuidString.lowercased())\"",
             replacement: "return \"signed-out-device\""),
    Mutation(name: "M04 accept noncanonical account spelling", rule: ownerRule,
             path: credentialScopePath,
             find: "rawValue == uuid.uuidString.lowercased()",
             replacement: "!rawValue.isEmpty"),
    ...
    Mutation(name: "M25 allow equal coordinator reload", rule: coordinatorRule, path: resolverPath,
             find: "guard revisionFence.accept(snapshot) else { return }",
    ...
    Mutation(name: "M39 drop rebuilt sync payload revision", rule: syncPayloadRule, path: syncPath,
             find: "debridRevision: doc.debridRevision,",
             replacement: "debridRevision: DebridCredentialSnapshotStore.shared.load().revision,"),
    ...
    Mutation(name: "M67 let older cloud recovery clear newer attempt ownership",
             ...
             find: "guard self.activeAttemptID == attemptID,\n                      self.credentialStore.load() == snapshot else { return }",
             replacement: "guard self.activeAttemptID == attemptID || true,\n                      self.credentialStore.load() == snapshot else { return }"),
    Mutation(name: "M68 bypass coordinator publication linearization", rule: coordinatorRule,
             path: resolverPath,
             find: "let current = credentialStore.compareAndInstall(revision: snapshot.revision) {",
             replacement: "let current = true\n        if true {"),
    Mutation(name: "M69 remove Trakt production Keychain default", rule: authCommitWiringRule,
             path: traktAuthPath,
             find: "tokenStorage: CredentialTokenStorage = .keychain,",
             replacement: "tokenStorage: CredentialTokenStorage,"),
    Mutation(name: "M70 bypass owner-scoped Keychain backend write", rule: authCommitWiringRule,
             path: credentialScopePath,
             find: "CredentialScopedKeychain.set(value, for: account, scope: scope)",
             replacement: "true"),
    Mutation(name: "M71 remove Trakt injected pre-commit suspension", rule: authCommitWiringRule,
             path: traktAuthPath,
             find: "if let onBeforeCommit { await onBeforeCommit() }",
             replacement: "_ = onBeforeCommit"),
]
