...
final class VortXSyncManager: ObservableObject {
    ...
    private func vortxSummary(existingVortx: [String: Any]? = nil) -> [String: Any] {
        ...
        //
        // READ-MERGE, never rebuild-from-local (#145 M6). `v` is built FRESH here and wholesale REPLACES
        // doc["vortx"] at the call site, so emitting only the LOCAL set silently DROPS every tombstone this
        // device has not folded: a device that never saw a peer's delete republishes a doc without that
        // tombstone, and the next peer union-merge RESURRECTS the deleted profile (with its watch overlay).
        // A device whose local set is empty (fresh install / reinstall) erased the account's whole set in one
        // push. UNION the pulled set instead, exactly like the apiKeys read-merge below and the addons union
        // above: this device may only ADD tombstones, never retract one another device authored. The owner id
        // is dropped defensively (it is never tombstoned; a stray one would erase the account owner).
        // Ids read back out of the doc are normalized to the uppercase `uuidString` form this set is keyed on
        // (UserProfile.normalizeID), exactly as Android's buildVortx normalizes the same read, so a
