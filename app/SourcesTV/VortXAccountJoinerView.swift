...
/// tvOS VortX-account sign-in by QR pairing (the JOINER side of VortXSyncManager.qrStart/qrPoll).
///
/// The Apple TV has no keyboard, so it signs into the VortX account by showing a QR + short code. The user
/// approves on a device already signed into VortX (a phone, or vortx.tv/approve in a browser), which hands
/// over the sync data key ECDH-wrapped to this TV's ephemeral key. The relay never sees the key. This signs
/// into the VortX account (the one that owns your add-ons, library, and sync), NOT a Stremio account.
struct VortXAccountJoinerView: View {
    /// Called on the main actor once this TV is signed into VortX and its account data has been pulled.
    var onSignedIn: () -> Void
