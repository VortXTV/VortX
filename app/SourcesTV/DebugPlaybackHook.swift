...
///
///   1. Launch environment (cold start, deterministic):
///        xcrun simctl launch --setenv VORTX_DEBUG_PLAY_URL=<url> <udid> com.stremiox.tv
///      Optional `VORTX_DEBUG_PLAY_TITLE` names the session (display only).
///   2. Deep link (warm re-trigger, no reinstall):
