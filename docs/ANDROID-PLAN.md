...
maintained by the **governing session** on branch `wip/vortx-android-plan-kiyl9y`; **worker
...
Worker branches: `wip/android-s<NN>-<slug>`. Every session ends with: builds green for **both
...
1. **Branch**: `wip/android-s<NN>-<slug>` off the current integration state (default branch
...
> `wip/android-s<NN>-<slug>`. Reference the Apple implementation and `docs/DESIGN-SYSTEM.md`
