...
SDKROOT="$(xcrun --sdk appletvos --show-sdk-path)" \
  cargo +nightly build $LOCKED $BUILDSTD --target aarch64-apple-tvos --release

echo "▸ tvOS simulator (aarch64-apple-tvos-sim)"
SDKROOT="$(xcrun --sdk appletvsimulator --show-sdk-path)" \
  cargo +nightly build $LOCKED $BUILDSTD --target aarch64-apple-tvos-sim --release

echo "▸ iOS device (aarch64-apple-ios)"
SDKROOT="$(xcrun --sdk iphoneos --show-sdk-path)" \
  cargo +nightly build $LOCKED --target aarch64-apple-ios --release

echo "▸ iOS simulator (aarch64-apple-ios-sim)"
SDKROOT="$(xcrun --sdk iphonesimulator --show-sdk-path)" \
  cargo +nightly build $LOCKED --target aarch64-apple-ios-sim --release
...
cargo +nightly build $LOCKED $BUILDSTD --target aarch64-apple-darwin --release
